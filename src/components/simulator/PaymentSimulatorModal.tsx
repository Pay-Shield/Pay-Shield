    ...
    try {
      ...
      if (createdTxn) {
        const updatedTxn: Transaction = {
          ...createdTxn,
          status: confirmed ? 'SAFE' : 'BLOCKED',
          hitlOutcome: confirmed ? 'confirmed' : 'cancelled',
        };
        setCreatedTxn(updatedTxn);
        onTransactionCreated(updatedTxn);
      }
    } catch (e) {
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs font-semibold text-blue-300 flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-blue-400 animate-pulse" />
            SIMULATION MODE — NO REAL MONEY IS TRANSFERRED
          </span>
          <span className="text-[10px] font-mono text-slate-400">TEST HARNESS</span>
        </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {presets.map((p) => (
                  <button
                    ...
                    className="p-2 rounded-lg bg-[#0B101D] hover:bg-[#131C2E] border border-slate-800 text-left transition-colors text-[11px] cursor-pointer"
                  >
                    <div className="font-semibold text-slate-200 truncate">{p.label}</div>
                    <div className="text-slate-400 text-[10px] font-mono tabular-nums">₹{p.amount.toLocaleString()}</div>
                  </button>
                ))}
              </div>
            </div>
        ...
        {stage === 'analyzing' && (
          <div className="py-8 px-4 flex flex-col items-center justify-center text-center">
            <div className="w-16 h-16 rounded-2xl bg-blue-600/10 border border-blue-500/30 flex items-center justify-center text-blue-400 mb-5 relative">
            ...
            <h3 className="text-lg font-bold text-white tracking-tight mb-1">
              PayShield is Evaluating This Payment
            </h3>
